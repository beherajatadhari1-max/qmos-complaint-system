      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        <div className="bg-blue-900 text-white px-6 py-4 rounded-t-2xl flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-300 text-xs mt-0.5">{process.clause}</p>
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1">Owner / Responsible Person</label>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button onClick={save} disabled={saving}
              className="flex-1 bg-blue-900 text-white py-2.5 rounded-xl text-sm font-bold hover:bg-blue-800 disabled:opacity-60">
      ...
      <div className="bg-white h-full w-full max-w-md shadow-2xl flex flex-col">
        <div className="bg-blue-900 text-white px-5 py-4 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-blue-300 text-xs mt-0.5">{process.docs.length} required documents</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {process.docs.map(doc => {
            ...
            return (
              <div key={doc} className="border border-gray-100 rounded-xl p-3 hover:border-blue-200 transition">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button onClick={() => { setActiveDoc(doc); fileRef.current?.click(); }}
                    className="text-xs bg-blue-50 text-blue-700 border border-blue-200 px-2 py-1 rounded-lg hover:bg-blue-100 flex-shrink-0">
                    ↑ Upload
                  </button>
                </div>
              </div>
        ...
        <div className="p-4 border-t border-gray-100 flex-shrink-0">
          <button onClick={onClose} className="w-full bg-blue-900 text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-blue-800">
            Close
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-blue-900 to-blue-700 text-white px-6 py-4 rounded-t-2xl flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-300 text-xs mt-0.5">Based on {relevant.length} logged records</p>
          </div>
          <button onClick={onClose} className="text-blue-300 hover:text-white text-2xl">×</button>
        </div>
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-4 gap-3">
            {[
              { label: 'Total', value: relevant.length, color: 'bg-blue-50 text-blue-800' },
              { label: 'Open', value: openCount, color: 'bg-red-50 text-red-700' },
              { label: 'Closed', value: closedCount, color: 'bg-green-50 text-green-700' },
              { label: 'Closure %', value: closureRate + '%', color: 'bg-purple-50 text-purple-700' },
            ].map(s => (
          ...
          <div className="bg-gray-50 rounded-xl p-3 flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500 font-semibold">30-Day Trend</p>
              <p className={`text-sm font-bold mt-0.5 ${trendColor}`}>{trend}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-gray-500">Last 30d: <b className="text-gray-800">{last30.length}</b></p>
              <p className="text-xs text-gray-500">Prev 30d: <b className="text-gray-800">{prev30.length}</b></p>
            </div>
          </div>
          {topDefects.length > 0 && (
            <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Top Defect Categories</p>
              <div className="space-y-1.5">
                {topDefects.map(([cat, cnt]) => (
                  <div key={cat} className="flex items-center gap-2">
                    <div className="text-xs text-gray-700 w-36 truncate">{cat}</div>
                    <div className="flex-1 bg-gray-100 rounded-full h-2">
                      <div className="bg-blue-600 h-2 rounded-full" style={{ width: `${(cnt / relevant.length) * 100}%` }} />
                    </div>
                    <span className="text-xs font-bold text-gray-600 w-5 text-right">{cnt}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {Object.keys(sevMap).length > 0 && (
            <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Severity Distribution</p>
              <div className="flex gap-2 flex-wrap">
                {Object.entries(sevMap).map(([sev, cnt]) => (
                  <span key={sev} className={`text-xs px-3 py-1.5 rounded-full font-semibold ${SEV_COLORS[sev] || 'bg-gray-100 text-gray-600'}`}>
                    {sev}: {cnt}
                  </span>
                ))}
              </div>
          ...
          {suggestions.length > 0 && (
            <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">💡 AI Recommendations</p>
              <div className="space-y-2">
                {suggestions.map((s, i) => (
                  <div key={i} className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs text-blue-900">{s}</div>
                ))}
              </div>
            </div>
          )}
  ...
  return (
    <div className="min-h-full bg-gray-50">
        ...
        <div className="grid grid-cols-4 gap-3 mt-4">
          {[
            ...
            <button key={s.label} onClick={() => handleFreqCard(s.freq)}
              ...
              <p className="text-xs text-white/60 mt-0.5">{freqFilter === s.freq ? '▲ Click to show all' : 'Click to filter →'}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm">
        {freqFilter && (
          <div className="px-4 py-1.5 bg-blue-50 border-b border-blue-100 flex items-center gap-3">
            <span className="text-xs text-blue-700 font-semibold">Showing: {freqFilter} activities only</span>
            <button onClick={() => { setFreqFilter(null); setActiveTab('__overview__'); }}
              className="text-xs text-blue-500 hover:text-blue-900 bg-white border border-blue-200 px-2 py-0.5 rounded-full">
            ...
            {!freqFilter && (
              <button onClick={() => setActiveTab('__overview__')}
                className={`flex flex-col items-start px-3 py-2.5 border-b-2 transition-all whitespace-nowrap text-left ${
                  activeTab === '__overview__' ? 'border-blue-900 text-blue-900 bg-blue-50' : 'border-transparent text-gray-500 hover:text-gray-800 hover:bg-gray-50'
                }`}>
                <div className="flex items-center gap-1.5">
                  <span className="text-sm">📋</span>
                  <span className="text-xs font-semibold">All Work Overview</span>
                </div>
                <span className="text-xs px-1.5 py-0.5 rounded border mt-1 font-medium bg-gray-100 text-gray-600 border-gray-200">{PROCESSES.length} activities</span>
              </button>
            )}
            {visibleProcesses.map(p => (
              <button key={p.id} onClick={() => setActiveTab(p.id)}
                className={`flex flex-col items-start px-3 py-2.5 border-b-2 transition-all whitespace-nowrap text-left group ${
                  activeTab === p.id ? 'border-blue-900 text-blue-900 bg-blue-50' : 'border-transparent text-gray-500 hover:text-gray-800 hover:bg-gray-50'
                }`}>
                <div className="flex items-center gap-1.5">
        ...
        {activeTab === '__overview__' && (
          <div className="space-y-4">
            <div className="bg-white rounded-xl shadow-sm p-5 border-l-4 border-blue-600">
              <h2 className="text-base font-bold text-gray-900 mb-1">📋 All Customer Quality Activities</h2>
              <p className="text-xs text-gray-500">Click any frequency card above to filter. Click any activity row to open it.</p>
            </div>
            {(['Daily','Weekly','Monthly','Quarterly'] as const).map(freq => {
              ...
              const freqStyle: Record<string,{hdr:string;badge:string;ring:string}> = {
                Daily:   { hdr:'bg-red-700',     badge:'bg-red-100 text-red-800 border-red-200',     ring:'border-red-200' },
                Weekly:  { hdr:'bg-blue-700',    badge:'bg-blue-100 text-blue-800 border-blue-200',   ring:'border-blue-200' },
                Monthly: { hdr:'bg-green-700',   badge:'bg-green-100 text-green-800 border-green-200',ring:'border-green-200' },
                Quarterly:{ hdr:'bg-purple-700', badge:'bg-purple-100 text-purple-800 border-purple-200',ring:'border-purple-200' },
              };
                ...
                <div key={freq} className={`bg-white rounded-xl shadow-sm overflow-hidden border ${st.ring}`}>
                  <div className={`${st.hdr} text-white px-5 py-3 flex items-center justify-between`}>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="bg-white/20 text-white text-xs px-2 py-0.5 rounded-full font-bold">{items.length} tasks</span>
                    </div>
                    <button onClick={() => handleFreqCard(freq)}
                      className="text-xs bg-white/20 hover:bg-white/30 text-white px-3 py-1 rounded-lg font-semibold">
                      Filter to {freq} only →
                    </button>
                  </div>
                  <div className="divide-y divide-gray-50">
                    {items.map((p, i) => {
                      ...
                      return (
                        <button key={p.id} onClick={() => setActiveTab(p.id)}
                          className="w-full flex items-center gap-4 px-5 py-3 hover:bg-blue-50 transition text-left group">
                          <span className="text-gray-400 text-xs font-mono w-5 flex-shrink-0">{i + 1}</span>
                          <span className="text-lg flex-shrink-0">{p.icon}</span>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-gray-900 group-hover:text-blue-900">{p.label}</p>
                            <p className="text-xs text-gray-400 truncate">{p.clause}</p>
                          </div>
                          {open > 0 && (
                            <span className="bg-red-100 text-red-700 text-xs px-2 py-0.5 rounded-full font-bold border border-red-200 flex-shrink-0">
                              {open} Open
                            </span>
                          )}
                          <span className={`text-xs px-2 py-0.5 rounded border font-medium flex-shrink-0 ${FREQ_COLORS[p.freq]}`}>{p.freq}</span>
                          <span className="text-blue-400 text-sm opacity-0 group-hover:opacity-100 flex-shrink-0">→</span>
                        </button>
                      );
                    })}
                  </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                {active.logType && openCount > 0 && (
                  <span className="bg-red-100 text-red-700 text-xs px-2 py-0.5 rounded-full font-semibold border border-red-200">{openCount} Open</span>
                )}
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
            <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full font-semibold">{tabComplaints.length} Total</span>
                {openCount > 0 && <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full font-semibold border border-red-200">{openCount} Open</span>}
              </div>
              <div className="flex items-center gap-2">
                <button onClick={refreshComplaints} className="text-gray-400 hover:text-blue-600 text-xs px-2 py-1.5 rounded hover:bg-gray-50">↻ Refresh</button>
                <button onClick={() => setModal(active.logType)}
                  className="bg-blue-900 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-blue-800 transition">
            ...
            <div className="bg-gradient-to-r from-blue-900 to-blue-700 text-white px-5 py-2.5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-blue-300 text-xs">— log a {active.logType} and 8D is auto-generated</span>
              </div>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {tabComplaints.map((c, idx) => (
                      <tr key={c.id} className={`hover:bg-blue-50 transition ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                        <td className="px-4 py-3"><span className="font-mono text-xs font-bold text-blue-900">{c.complaint_number || `#${c.id}`}</span></td>
                        <td className="px-4 py-3 text-xs text-gray-600 whitespace-nowrap">{c.created_at?.slice(0, 10) || '—'}</td>
                        <td className="px-4 py-3 text-xs font-medium text-gray-800 max-w-[120px] truncate">{c.customer_name}</td>
                        ...
                        {active.logType === 'Warranty' && <td className="px-4 py-3 text-xs text-gray-600 font-mono">{c.warranty_claim_no || '—'}</td>}
                        {active.logType === 'Customer Concern' && <td className="px-4 py-3 text-xs text-gray-600 font-mono">{c.prr_number || '—'}</td>}
                        {active.logType === 'Customer Rejection' && <td className="px-4 py-3 text-xs text-gray-600">{c.rejection_stage || '—'}</td>}
                        <td className="px-4 py-3 text-xs text-gray-700 max-w-[160px]">
                          <div className="font-medium">{c.defect_category}</div>
                          <div className="text-gray-500 line-clamp-1">{c.defect_description}</div>
                        </td>
                        <td className="px-4 py-3 text-center text-xs font-bold">{c.quantity_affected ?? 0}</td>
                        <td className="px-4 py-3 text-center">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${SEV_COLORS[c.severity] || 'bg-gray-100 text-gray-600'}`}>{c.severity}</span>
                        </td>
                        <td className="px-4 py-3 text-center">
                        ...
                        <td className="px-4 py-3 text-center">
                          {c.report_generated ? (
                            <a href={`/complaints/${c.id}`} className="inline-flex items-center gap-1 bg-blue-900 text-white text-xs px-2.5 py-1.5 rounded-lg font-semibold hover:bg-blue-800 transition">📊 Open 8D</a>
                          ) : (
              ...
              {active.activities.map((act, i) => (
                <div key={i} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100 hover:border-blue-200 hover:bg-blue-50 transition group">
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 text-xs text-blue-700 bg-blue-100 hover:bg-blue-200 px-2 py-1 rounded-lg font-semibold flex-shrink-0 transition">
                    Log ›
                  </button>
                </div>
              ))}
                ...
                {active.kpis.map((kpi, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between p-2.5 bg-gray-50 rounded-lg">
                      ...
                      <span className={`text-xs font-bold ${value !== '—' ? 'text-blue-700' : 'text-gray-400'}`}>{value}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <button key={i} onClick={s.action}
                    className="w-full text-left text-xs text-blue-100 hover:text-white bg-blue-800 hover:bg-blue-700 px-3 py-2 rounded-lg transition">
                    💡 {s.label}
                  </button>
                ))}
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setActivityModal({ process: active })}
                  className="bg-blue-900 text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-blue-800 transition">
                  + Add Entry
                </button>
              </div>
            </div>
                ...
                {filteredLogs.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-12 text-center">
                      <p className="text-gray-400 text-sm mb-3">No activity logged yet for this {active.freq.toLowerCase()} process.</p>
                      <button onClick={() => setActivityModal({ process: active })}
                        className="inline-flex items-center gap-2 bg-blue-900 text-white px-5 py-2 rounded-lg text-sm font-semibold hover:bg-blue-800 transition">
                        {active.icon} Log First Entry →
                      </button>
                    </td>
                  </tr>
                ) : filteredLogs.map((log, idx) => (
                  <tr key={log.id} className={`hover:bg-blue-50 transition ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}`}>
                    <td className="px-4 py-3 text-xs text-gray-600 whitespace-nowrap">{log.log_date || '—'}</td>
                    <td className="px-4 py-3 text-xs text-gray-800 max-w-[200px]"><span className="font-medium">{log.activity_step}</span></td>
                    ...
                    <td className="px-4 py-3 text-xs text-blue-700 max-w-[130px] truncate">{log.evidence || '—'}</td>
