...
const DEFECT_TYPES = [
  ...
  { type:'Major (B)', def:'Defect affecting function — part will not perform its intended purpose', action:'Reject lot — SCAR to supplier — 8D required', color:'bg-orange-100 border-orange-300 text-orange-800' },
  { type:'Minor (C)', def:'Defect not affecting function — cosmetic or aesthetic deviation', action:'Apply AQL — disposition on severity and quantity', color:'bg-yellow-100 border-yellow-300 text-yellow-800' },
];
...
export default function IncomingQualityPage() {
  const [tab, setTab] = useState<'tracker'|'knowledge'|'aql'>('tracker');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ── HEADER ─────────────────────────────────────────────────────── */}
      <div className="bg-teal-900 text-white px-6 py-5">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex gap-1 mt-4">
            {([['tracker','📋 Process Tracker'],['knowledge','📚 IQC Knowledge Hub'],['aql','📐 AQL & Inspection Guide']] as const).map(([id,label])=>(
              <button key={id} onClick={()=>setTab(id)}
                className={`px-4 py-2 rounded-t-lg text-sm font-semibold transition ${tab===id ? 'bg-white text-teal-900' : 'text-teal-300 hover:text-white'}`}>
                {label}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto p-5">
        ...
        {tab === 'tracker' && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-wide">Filter by Frequency:</span>
              {freqs.map(f => (
                <button key={f} onClick={()=>setFreqFilter(f)}
                  className={`text-xs px-3 py-1.5 rounded-full font-semibold transition ${freqFilter===f ? 'bg-teal-700 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
                  {f}
                </button>
              ))}
            ...
            {filtered.map(p => (
              <div key={p.no} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                {expanded === p.no && (
                  <div className="border-t border-gray-100 px-5 py-4 space-y-4 bg-teal-50/30">
        ...
        {tab === 'knowledge' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <h2 className="text-lg font-bold text-gray-800 mb-4">🎯 What is Incoming Quality Control (IQC)?</h2>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <h2 className="text-lg font-bold text-gray-800 mb-4">🔴 Defect Classification — IQC Decision Guide</h2>
              ...
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                <p className="text-sm font-bold text-blue-900 mb-1">💡 IQC Golden Rule</p>
                <p className="text-sm text-blue-800">When in doubt about defect classification — <strong>always escalate to Quality Head</strong>. Never accept a critical defect under AQL. Critical defects always require 100% inspection.</p>
              </div>
            </div>

            {/* Inspection Flow */}
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <h2 className="text-lg font-bold text-gray-800 mb-4">🔄 IQC Inspection Flow — Step by Step</h2>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                {FLOW_STEPS.map((s,i) => (
                  <div key={i} className="relative">
                    <div className="bg-teal-900 text-white rounded-xl p-3 text-center h-full">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <h2 className="text-lg font-bold text-gray-800 mb-4">🔬 Common Incoming Defects by Material Type</h2>
            ...
            {/* IATF Requirements */}
            <div className="bg-teal-950 rounded-xl p-6 text-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className="bg-teal-900/50 rounded-lg p-4 border border-teal-700">
        ...
        {tab === 'aql' && (
          <div className="space-y-6">
            ...
            {/* AQL Intro */}
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <h2 className="text-lg font-bold text-gray-800 mb-3">📐 What is AQL?</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-xl">
                    <p className="text-sm font-bold text-yellow-800">⚠️ AQL is NOT 100% protection</p>
                    <p className="text-sm text-yellow-700 mt-1">AQL gives statistical confidence, not a guarantee. A lot can be accepted under AQL even if it has some defects. For critical characteristics — always inspect 100%.</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                    <p className="font-bold text-blue-800 text-sm mb-2">📊 Common AQL Levels Used in Automotive</p>
                    <div className="space-y-1">
                      {[
                        ...
                        <div key={i} className="flex gap-3 text-xs">
                          <span className="font-mono font-bold text-blue-700 w-20 flex-shrink-0">{a.level}</span>
                          <span className="text-blue-700">{a.use}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                    <p className="font-bold text-green-800 text-sm mb-2">📋 Inspection Levels</p>
            ...
            {/* AQL Table */}
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <h2 className="text-lg font-bold text-gray-800 mb-1">📋 AQL Sampling Table — Level II, AQL 1.0 (Major Defects)</h2>
              <p className="text-xs text-gray-500 mb-4">Based on ANSI/ASQ Z1.4. Ac = Accept if defects ≤ this number. Re = Reject if defects ≥ this number.</p>
                ...
                <table className="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                  <thead className="bg-teal-900 text-white">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-red-300">Reject if defects ≥ (Re)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {AQL_TABLE.map((row,i)=>(
                      <tr key={i} className={i%2===0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="px-4 py-2.5 font-mono text-sm text-gray-700">{row.lot}</td>
                        <td className="px-4 py-2.5 font-bold text-gray-800">{row.code}</td>
                        <td className="px-4 py-2.5 font-bold text-blue-700">{row.n}</td>
                        <td className="px-4 py-2.5 font-bold text-green-700">{row.ac}</td>
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={i} className="bg-blue-50 border border-blue-200 rounded-xl p-3">
                    <p className="font-bold text-blue-800 text-sm mb-1">{e.title}</p>
                    <p className="text-xs text-blue-700">{e.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Gauge Guide */}
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <h2 className="text-lg font-bold text-gray-800 mb-4">🔬 IQC Gauge & Measurement Quick Guide</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={i} className="border border-gray-200 rounded-xl overflow-hidden">
                    <div className="bg-gray-800 text-white px-4 py-2">
                      ...
                      <p className="text-xs text-gray-300">Use: {g.use}</p>
                    </div>
                    <div className="p-3 bg-gray-50">
                      <p className="text-xs text-gray-700"><strong>Rule:</strong> {g.rule}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-teal-950 rounded-xl p-6 text-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <div className="bg-teal-900/60 rounded-xl p-5 text-center border border-teal-700 mb-4">
                  ...
                  <div className="bg-teal-900/40 rounded-xl p-4 border border-teal-700">
