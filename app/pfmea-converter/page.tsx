...
function apBadge(ap: string) {
  ...
  return <span className="text-[#1e3a5f] text-xs">â€”</span>;
}
...
export default function PFMEAConverterPage() {
  ...
  const ta = 'bg-white border border-[#dbeafe] text-[#1e3a5f] text-xs px-1 py-0.5 rounded w-full min-h-[40px] resize-y';
  ...
  return (
    <div className="min-h-screen bg-white text-[#1e3a5f] p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-orange-600">ðŸ”„ Old AIAG FMEA â†’ New AIAG VDA 2019 Converter</h1>
        <p className="text-[#1e3a5f] text-sm mt-1">Convert your old RPN-based FMEA to new Action Priority (AP) format per AIAG VDA Handbook 1st Edition 2019</p>
      </div>

      <div className="overflow-x-auto flex gap-2 mb-4 border-b border-[#dbeafe]">
        <button onClick={() => setTab('manual')} className={`px-4 py-2 text-sm font-medium rounded-t border-b-2 transition-colors ${tab==='manual'?'border-orange-400 text-orange-600 bg-white':'border-transparent text-[#1e3a5f] hover:text-[#1e3a5f]'}`}>
          âœï¸ Manual Entry
        </button>
        <button onClick={() => setTab('upload')} className={`px-4 py-2 text-sm font-medium rounded-t border-b-2 transition-colors ${tab==='upload'?'border-orange-400 text-orange-600 bg-white':'border-transparent text-[#1e3a5f] hover:text-[#1e3a5f]'}`}>
      ...
      {tab === 'upload' && (
        <div className="animate-fadeIn space-y-4">
          <div className="bg-white border-2 border-dashed border-[#dbeafe] rounded-xl p-8 text-center">
            <div className="text-4xl mb-3">ðŸ“Š</div>
            <p className="text-white font-medium mb-1">Upload Old AIAG FMEA Excel File</p>
            <p className="text-[#1e3a5f] text-xs mb-4">Supports .xlsx and .xls â€” Old format (Pre-2019) with RPN columns</p>
            ...
            {uploadFileName && <p className="text-[#1e3a5f] text-xs mt-3">ðŸ“Ž {uploadFileName}</p>}
          </div>

          {uploadStatus && (
            <div className={`text-xs p-3 rounded border ${uploadStatus.startsWith('Loaded')?'bg-green-900/30/20 border-green-700 text-green-300':'bg-red-50 border-red-700 text-red-700'}`}>
              {uploadStatus}
            </div>
          )}
          ...
          {headers.length > 0 && (
            <div className="bg-white rounded-lg p-4 border border-[#dbeafe]">
              <h3 className="text-sm font-bold text-orange-600 mb-1 uppercase tracking-wide">Column Mapping</h3>
              <p className="text-[#1e3a5f] text-xs mb-4">Auto-detected from your Excel headers. Correct any wrong mappings before importing.</p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={field} className="flex flex-col gap-1">
                    <label className="text-xs text-[#1e3a5f] font-medium">{label}</label>
                    <select value={colMap[field]||''} onChange={e=>setColMap(m=>({...m,[field]:e.target.value}))} className="bg-gray-700 border border-[#dbeafe] text-white text-xs px-2 py-1 rounded">
                      <option value="">â€” Not mapped â€”</option>
                      {headers.map(h=><option key={h} value={h}>{h}</option>)}
                    </select>
                    {colMap[field] && <span className="text-green-600 text-xs">âœ“ {colMap[field]}</span>}
                  </div>
                ))}
              </div>
              ...
              {previewData.length > 0 && (
                <div className="mt-4">
                  <p className="text-xs text-[#1e3a5f] mb-2">Preview (first 5 rows):</p>
                  <div className="overflow-x-auto">
                    <table className="text-xs border-collapse min-w-max">
                      <thead><tr>{headers.map(h=><th key={h} className="px-2 py-1 bg-gray-700 border border-[#dbeafe] text-[#1e3a5f] whitespace-nowrap">{h}</th>)}</tr></thead>
                      <tbody>{previewData.map((row,i)=><tr key={i}>{headers.map((_,j)=><td key={j} className="px-2 py-0.5 border border-[#dbeafe] text-[#1e3a5f] whitespace-nowrap max-w-xs truncate">{String(row[j]??'')}</td>)}</tr>)}</tbody>
                    </table>
                  </div>
                </div>
              )}
              ...
              <div className="mt-4 flex gap-3 items-center">
                ...
                <span className="text-[#1e3a5f] text-xs">After import, review in Manual Entry tab then click Convert</span>
              </div>
            </div>
          )}

          <div className="bg-[#eff6ff] border border-blue-700/50 rounded p-3 text-xs text-blue-200">
            ...
            <p className="mt-1 text-[#1d4ed8]">Column names are auto-detected â€” works even if your headers are slightly different.</p>
          </div>
        </div>
      )}
      ...
      {tab === 'manual' && (
        <div className="animate-fadeIn mb-4">
          <div className="bg-white rounded-lg p-4 mb-4 border border-[#dbeafe]">
            <h2 className="text-sm font-bold text-orange-600 mb-2 uppercase tracking-wide">What changes in the conversion?</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
              <div className="bg-red-50 border border-red-700 rounded p-3">
                <p className="font-bold text-red-700 mb-1">Removed (Old Format)</p>
                <p className="text-[#1e3a5f]">RPN = S x O x D (Risk Priority Number)</p>
                <p className="text-[#1e3a5f]">Single threshold e.g. RPN &gt; 100</p>
                <p className="text-[#1e3a5f]">No structure/function analysis</p>
              </div>
              <div className="bg-green-900/30 border border-green-700 rounded p-3">
                <p className="font-bold text-[#15803d] mb-1">Added (New AIAG VDA 2019)</p>
                <p className="text-[#1e3a5f]">AP = H/M/L (Action Priority table)</p>
                <p className="text-[#1e3a5f]">Structure + Function Analysis columns</p>
                <p className="text-[#1e3a5f]">7-Step approach</p>
                <p className="text-[#1e3a5f]">Work Element (4M Type)</p>
              </div>
              <div className="bg-[#eff6ff] border border-blue-700/50 rounded p-3">
                <p className="font-bold text-[#1d4ed8] mb-1">Auto-Mapped Fields</p>
                <p className="text-[#1e3a5f]">Item/Function to Process Step</p>
                <p className="text-[#1e3a5f]">Class (CC/SC) to Special Characteristics</p>
                <p className="text-[#1e3a5f]">Recommended Action to Prevention Action</p>
                <p className="text-[#1e3a5f]">S, O, D to AP (auto-calculated)</p>
              </div>
            </div>
          </div>

          <div className="flex items-center justify-between mb-2 flex-wrap gap-y-2">
            <h2 className="text-sm font-bold text-red-700 uppercase tracking-wide">Step 1: Enter Old AIAG FMEA Data (Pre-2019 Format)</h2>
            <button onClick={addRow} className="bg-gray-700 hover:bg-gray-600 text-white text-xs px-3 py-1.5 rounded">+ Add Row</button>
          </div>
          <div className="overflow-x-auto rounded-lg border border-[#dbeafe] mb-3">
            <table className="text-xs min-w-max border-collapse">
              <thead>
                <tr>
                  ...
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-32`}>Recommended{'\n'}Action</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-24`}>Responsibility</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-24`}>Target Date</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-32`}>Action Taken</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-8`}>S'</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-8`}>O'</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-8`}>D'</th>
              ...
              <tbody>
                {oldRows.map((r, i) => (
                  <tr key={r.id} className={i%2===0?'bg-white':'bg-[#eff6ff]'}>
                    ...
                    <td className={td}><button onClick={()=>delRow(r.id)} className="text-red-600 hover:text-red-200 text-sm px-1">x</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-[#1e3a5f] text-xs">S, O, D values auto-calculate ACtion Priority (AP) per AIAG VDA table</span>
          </div>
        </div>
      )}
      ...
      {converted && newRows.length > 0 && (
        <div className="mt-6">
          <div className="flex items-center justify-between mb-2 flex-wrap gap-y-2">
            <h2 className="text-sm font-bold text-[#15803d] uppercase tracking-wide">Step 2: Converted AIAG VDA 2019 PFMEA</h2>
          ...
          <div className="overflow-x-auto rounded-lg border border-[#dbeafe]">
            <table className="text-xs min-w-max border-collapse">
              <thead>
                <tr>
                  <th colSpan={2} className={`${th} bg-[#eff6ff] text-blue-200`}>STRUCTURE ANALYSIS</th>
                  <th colSpan={2} className={`${th} bg-green-900/30 text-green-200`}>FUNCTION ANALYSIS</th>
                  ...
                  <th colSpan={10} className={`${th} bg-purple-900/30 text-purple-200`}>OPTIMIZATION</th>
                </tr>
                <tr>
                  <th className={`${th} bg-[#eff6ff] text-blue-200 w-32`}>Process Step</th>
                  <th className={`${th} bg-[#eff6ff] text-blue-200 w-24`}>Work Element{'\n'}(4M Type)</th>
                  <th className={`${th} bg-green-900/30 text-green-200 w-32`}>Function of{'\n'}Process Step</th>
                  <th className={`${th} bg-green-900/30 text-green-200 w-32`}>Function of{'\n'}Work Element</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-36`}>Failure Effects (FE)</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-8`}>S</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-32`}>Failure Mode (FM)</th>
                  <th className={`${th} bg-orange-900/30 text-orange-200 w-32`}>Failure Cause (FC)</th>
                  ...
                  <th className={`${th} bg-purple-900/30 text-purple-200 w-28`}>Prevention{'\n'}Action</th>
                  <th className={`${th} bg-purple-900/30 text-purple-200 w-28`}>Detection{'\n'}Action</th>
                  <th className={`${th} bg-purple-900/30 text-purple-200 w-20`}>Responsible</th>
                  <th className={`${th} bg-purple-900/30 text-purple-200 w-20`}>Target{'\n'}Date</th>
                  <th className={`${th} bg-purple-900/30 text-purple-200 w-20`}>Status</th>
                  <th className={`${th} bg-purple-900/30 text-purple-200 w-28`}>Action Taken</th>
                  ...
                  <th className={`${th} bg-purple-900/30 text-purple-200 w-28`}>Remarks</th>
                </tr>
              </thead>
              <tbody>
                {newRows.map((r, i) => (
                  <tr key={r.id} className={i%2===0?'bg-white':'bg-[#eff6ff]'}>
                    ...
                    <td className={td}><span className={`text-xs font-medium ${r.status==='Completed'?'text-green-600':'text-yellow-600'}`}>{r.status}</span></td>
                    ...
                    <td className={td}><span className="text-[#1e3a5f] text-xs">{r.remarks}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="mt-3 bg-yellow-900/30 border border-yellow-700 rounded p-3 text-xs text-yellow-200">
