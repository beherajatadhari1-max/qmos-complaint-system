...
export default function PFMEAConverterPage() {
  ...
  const handleConvert = () => {
    setNewRows(oldRows.map(convertRow));
    setConverted(true);
  };
  ...
  const exportExcel = async () => {
    ...
    if (converted && newRows.length) {
      ...
      ws2.addRow([]);
      ws2.addRow(['STRUCTURE ANALYSIS','','FUNCTION ANALYSIS','','FAILURE ANALYSIS','','','','RISK ANALYSIS','','','','','','OPTIMIZATION','','','','','','','','','','','']);
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4">
      ...
      <div className="bg-gray-800 rounded-lg p-4 mb-4 border border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
          <div className="bg-red-900/30 border border-red-700 rounded p-3">
            ...
            <p className="text-gray-300">• RPN = S × O × D (Risk Priority Number)</p>
            <p className="text-gray-300">• Single threshold (e.g. RPN &gt; 100 = Act)</p>
          ...
          <div className="bg-blue-900/30 border border-blue-700 rounded p-3">
            ...
            <p className="text-gray-300">• Item/Function → Process Step</p>
            <p className="text-gray-300">• Class (CC/SC) → Special Characteristics</p>
            <p className="text-gray-300">• Recommended Action → Prevention Action</p>
            <p className="text-gray-300">• S, O, D → AP (auto-calculated)</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-4">
          ...
          <table className="text-xs min-w-max border-collapse">
            <thead>
              <tr>
                ...
                <th className={`${th} bg-red-900 text-red-200 w-36`}>Failure Effect{'→'}Customer</th>
                ...
                <th className={`${th} bg-red-900 text-red-200 w-14`}>RPN{'\n'}(S×O×D)</th>
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-gray-400 text-xs">S, O, D values will be used to auto-calculate Action Priority (AP) per AIAG VDA table</span>
        </div>
      </div>
      ...
      {converted && newRows.length > 0 && (
        <div className="mt-6">
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-700">
            <table className="text-xs min-w-max border-collapse">
              <thead>
                ...
                <tr>
                  <th className={`${th} bg-blue-950 text-blue-200 w-32`}>Process Step{'\n'}(Station/Focus)</th>
                  <th className={`${th} bg-blue-950 text-blue-200 w-24`}>Work Element{'\n'}(4M Type)</th>
                  <th className={`${th} bg-green-950 text-green-200 w-32`}>Function of{'\n'}Process Step</th>
                  <th className={`${th} bg-green-950 text-green-200 w-32`}>Function of{'\n'}Work Element</th>
                  <th className={`${th} bg-orange-950 text-orange-200 w-36`}>Failure Effects (FE){'\n'}↑ Customer</th>
                  ...
                  <th className={`${th} bg-purple-950 text-purple-200 w-28`}>Action Taken{'\n'}(+ Evidence)</th>
                  ...
                  <th className={`${th} bg-purple-950 text-purple-200 w-28`}>Remarks{'\n'}(Old RPN ref)</th>
                </tr>
              </thead>
              <tbody>
                {newRows.map((r, i) => (
          ...
          <div className="mt-3 bg-yellow-900/20 border border-yellow-700 rounded p-3 text-xs text-yellow-200">
            <p className="font-bold mb-1">⚠️ Manual review required after conversion:</p>
            <p>1. Fill in <strong>Work Element (4M Type)</strong> column — Man / Machine / Material / Method</p>
            <p>2. Fill in <strong>Function of Work Element</strong> — describe specific work element function</p>
            <p>3. Verify <strong>AP rating</strong> — if AP differs from old RPN priority, review with core team</p>
            <p>4. Split <strong>Recommended Action</strong> into Prevention Action vs Detection Action as needed</p>
          </div>
        </div>
      )}
    </div>
